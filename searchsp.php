...
<head>
 ...
 <style>
   ...
   .items{
    margin-left:180px;width:1180px; background-color: #00000021;height:1280px;margin-top:0px;padding-left:45px;
   }
...
if (empty($search)) {
?>
  <div class="notification" > <i class="fa fa-question-circle" aria-hidden="true" style="color:white;"></i><?php echo " Yêu cầu nhập tên sản phẩm cần tìm kiếm !";?></div>
<?php
} else {

// Ngược lại nếu người dùng nhập liệu thì tiến hành xứ lý show dữ liệu.

// Nếu $num > 0 hoặc $search khác rỗng tức là có dữ liệu mối show ra , ngược lại thì báo lỗi.
...
?>
    <div class="notification" > <i class="fa fa-check" aria-hidden="true" style="color:white;"></i><?php echo " Có $num sản phẩm phù hợp với từ khóa cần tìm kiếm '<b>$search</b>'";?></div>
...
?>
    <div class="notification" > <i class="fa fa-low-vision" aria-hidden="true" style="color:white"></i><?php echo " Không tìm thấy sản phẩm !!!";?></div>
...
?>  
   ...
   <!-- het phan hien thi -->
	<!-- phan pagination -->
  <div class="pagination" style="  display: flex;justify-content: center; margin:500px 0px 50px 250px;font-size:30px;background-color:#000000cc; width:1055px;border-radius:8px;">
    <?php
        for($i=1;$i<=$maxpage;$i++){
          
            if($i==$page &&  $ppage<=13){
                echo "<a style = 'text-align: center;color:orangered;font-size: 20'><i class='fa fa-circle 'style='font-size:8px;' aria-hidden='true'></i> $i <i class='fa fa-circle 'style='font-size:8px;' aria-hidden='true'></i></a> " ;
                 break;
              }else{
                if($i==$page){
                  echo "<a style = 'color:orangered;font-size: 20'><i class='fa fa-circle 'style='font-size:8px;' aria-hidden='true'></i> $i <i class='fa fa-circle 'style='font-size:8px;' aria-hidden='true'></i></a> " ;
                   
                }
                else if($i==$maxpage){	
                  			
                    echo "<a href = 'searchsp.php?p=$i' > Trang cuối <i class='fa fa-arrow-circle-right' aria-hidden='true'></i></a><br>" ;
                }else if($i==1){
                    echo "<a href = 'searchsp.php?p=$i'><i class='fa fa-arrow-circle-left' aria-hidden='true'></i> Trang đầu </a> " ;
                
                }else{
                    if(($i>=$page-5) AND ($i<=$page+5))
                    echo "<a href = 'searchsp.php?p=$i'>$i</a> " ;
                }
            }
        }
    ?>
